...
public class SentimentAnalyzer {
    ...
    private static int checkForWasPhrasePattern(String review, String feature, String[] posOpinionWords, String[] negOpinionWords) {
        ...
        for (String posOpinionWord : posOpinionWords) {
            StringBuilder patternOpinion = new StringBuilder(pattern);
            patternOpinion.append(posOpinionWord);

            if (review.contains(patternOpinion)) {
                return ++opinion;
            }

        }
        for(String negOpinionWord : negOpinionWords){
            String patternOpinion = pattern + negOpinionWord;
    ...
    private static int checkForOpinionFirstPattern(String review, String feature, String[] posOpinionWords,
        ...
        for (String sentence : sentences) {
            ...
            for (String posOpinionWord : posOpinionWords) {
                String patternOpinion = posOpinionWord + ' ' + feature;

                if (sentence.contains(patternOpinion)) {
                    opinion++;
                }
            ...
            for (String negOpinionWord : negOpinionWords) {
                String patternOpinion = negOpinionWord + ' ' + feature;

                if (sentence.contains(patternOpinion)) {
                    opinion--;
                }
            }
            //break; // so it should process only a sentence
        }
        return opinion;
    }
